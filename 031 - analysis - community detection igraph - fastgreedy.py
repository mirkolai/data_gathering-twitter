...
graph=igraph.Graph.TupleList(records, weights=True, directed=False)
#fast-greedy community finding works only on graphs without multiple edges, Invalid value
graph.simplify(combine_edges=dict(weight="sum"))
print "Is graph weighted? ",graph.is_weighted()

community_fastgreedy = graph.community_fastgreedy( weights='weight')
print community_fastgreedy.as_clustering().summary()
is_edges_between_clusters = community_fastgreedy.as_clustering().crossing()

print "Finding edges between and not-between communities"
link_internal=[]
link_external=[]
print 'Number of edges', len(is_edges_between_clusters)
...
for r in records:
    ...
    if r[0]==r[1]: #recover self-loop even is difficult that a user reply to himself
        cur.execute("INSERT is_edge_between (tweet_id, fastgreedy) "
            "VALUES (%s,%s) "
            "on duplicate key update fastgreedy=%s",
            (r[2], 1, 1) #set in the same community
            )
        db.commit()
...
for community in community_fastgreedy.as_clustering():
    color = "#%06x" % random.randint(0, 0xFFFFFF)
    for vertex in community:
        graph.vs(vertex)['color']=color
# plot graph
igraph.plot(graph)
